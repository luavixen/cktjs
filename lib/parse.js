...
/** Matches the body of each line of a multiline string expression [, body, newline] */
var multilineRegex =/[ \t\v\f\r]*\|(.*)(\n|$)/g;
...
/**
 * Parses a CKT literal's body and attempts to convert it to a number/boolean/null
 * @param {string} string
...
export function parse(text) {
  ...
  /**
   * Token string value
   *  - Contains the string body for string/multiline
  ...
  function tkNext() {
    ...
    if (result) {
      ...
      // Extract line bodies from multiline strings
      if (type > /*STRING*/5 /* type === MULTILINE */) {
        var lines = [];
        while (result = multilineRegex.exec(string)) {
          lines.push(result[1]);
          if (result[2]) {
            line++;
          }
  ...
  function tkSlurp(separators) {
    var matched = false;
    while (tkMatch(/*NEWLINE*/1) || (separators && tkMatch(/*SEPARATOR*/2))) {
      matched = true;
    }
    return matched;
  }
